...
if ($int_wp_version >= 390000000 && 		// Wordpress version > 3.9
    ...
    defined('WPINC')) {				// Plugin is not loaded directly
        ...
        if (in_array( 'fastmember/fastmember.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
            require_once( FASTCF_DIR . '/lib/fast-cf-main-class.php' );
            $fcf = new fast_CF_Main_Class();
            add_action('init', 'fastcf_ipn_handle');
        }
...
function fastcf_ipn_handle() {
    ...
    if ( isset( $_POST["charge_id"]) && isset( $_POST["original_amount_cents"]) && $_POST["status"] == "paid" ) {
        require_once( FASTCF_DIR . '/lib/fast-cf-ipn-class.php' );
        $fcfipn = new fast_CF_IPN_Class();
    }
